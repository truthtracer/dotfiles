...
function s:exJS_PushJumpStack( state ) " <<<
...
function s:exJS_SetLastJumpStack( state ) " <<<
...
function s:exJS_PushEntryState ( state ) " <<<
...
function s:exJS_ClearEntryStateList () " <<<
    if !empty(s:exJS_entry_list)
        silent call remove ( s:exJS_entry_list, 0, len(s:exJS_entry_list)-1 )
    endif
endfunction " >>>
...
function s:exJS_InitSelectWindow() " <<<
...
function s:exJS_UpdateSelectWindow() " <<<
